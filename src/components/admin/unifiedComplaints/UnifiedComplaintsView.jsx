...
import { MapPin, Calendar, User, FileText, Bot, Eye, Brain, Merge, UserCheck, AlertTriangle, Clock, CheckCircle } from 'lucide-react';

const UnifiedComplaintsView = ({ allocatedComplaints = [], complaints = [] }) => {
  ...
  const mergedComplaints = useMemo(() => {
    return allocatedComplaints.map(allocated => {
      ...
      const originalComplaint = complaints.find(original => {
        ...
        if (allocated.id && original.id && allocated.id === original.id) return true;
        
        // If no ID match, try to match by title and user email
        if (allocated.title && original.title && 
            allocated.userInfo?.email && original.contact?.email) {
          return allocated.title === original.title && 
                 allocated.userInfo.email === original.contact.email;
        }
        
        // Fallback: match by title and timestamp proximity (within 1 hour)
        if (allocated.title && original.title && allocated.title === original.title) {
          const allocatedTime = new Date(allocated.createdAt || allocated.timestamp);
          const originalTime = new Date(original.timestamp || original.createdAt);
  ...
  const updateComplaintStatus = async (complaintId, newStatus) => {
    ...
    try {
      ...
      const complaintToUpdate = updatedAllocated.find(c => c.id === complaintId);
      if (complaintToUpdate) {
        await fetch(`http://localhost:${port}/allocatedDepartment/${complaintId}`, {
          method: 'PUT',
      ...
      const originalComplaintsResponse = await fetch(`http://localhost:${port}/complaints`);
      const originalData = await originalComplaintsResponse.json();
      const originalComplaint = originalData.find(c => c.id === complaintId || c.title === complaintToUpdate?.title);
      
      if (originalComplaint) {
        await fetch(`http://localhost:${port}/complaints/${originalComplaint.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
    ...
    try {
      ...
      if (originalComplaint) {
        await fetch(`http://localhost:${port}/complaints/${originalComplaint.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...originalComplaint, priority: newPriority })
        });
      }
  ...
  const assignStaff = async (complaintId, staffId, isAutoAssignment = false) => {
    ...
    try {
      ...
      const updatedAllocated = allocatedData.map(complaint => 
        complaint.id === complaintId ? { ...complaint, assignedTo: staffId } : complaint
      );
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4">
        ...
        <p className="text-indigo-700 text-sm mt-1">
          AI-processed complaints merged with original data for complete context and easy management.
        </p>
      </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{mergedComplaints.length}</p>
            </div>
          </div>
        </div>
            ...
            {mergedComplaints.map((complaint, idx) => (
              ...
              >
                ...
                <div className="absolute top-2 right-2 flex space-x-1">
                  <div className="w-2 h-2 bg-indigo-500 rounded-full" title="AI Processed"></div>
                  {complaint.hasOriginalData && (
                    <div className="w-2 h-2 bg-blue-500 rounded-full" title="Original Data Available"></div>
                  )}
                </div>
                ...
                <p className="text-gray-600 text-sm mb-3 line-clamp-2">
                  {complaint.analysis || complaint.details || complaint.originalDescription}
                </p>
                ...
                <div className="flex items-center text-xs text-gray-500 mb-2">
                  ...
                  {complaint.location?.municipality || complaint.originalLocation?.municipality || "Unknown location"}
                </div>
                ...
                {complaint.confidence && (
                  <div className="mt-2 bg-indigo-50 px-2 py-1 rounded text-xs">
                    <span className="text-indigo-700">AI Confidence: {Math.round(complaint.confidence * 100)}%</span>
                  </div>
                )}
            ...
            <div className="p-6">
              ...
              {(selectedComplaint.photo || selectedComplaint.originalPhoto) && (
                <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedComplaint.photo && (
                    <div>
                      <h4 className="text-sm font-medium text-gray-600 mb-2">AI Processed Image</h4>
                      <img
                  ...
                  {selectedComplaint.originalPhoto && selectedComplaint.originalPhoto !== selectedComplaint.photo && (
                    <div>
                      <h4 className="text-sm font-medium text-gray-600 mb-2">Original Submission</h4>
                      <img
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-indigo-900 border-b pb-2">AI Analysis & Processing</h3>
                  ...
                  {selectedComplaint.questionnaire && selectedComplaint.questionnaire.length > 0 && (
                    <div>
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-blue-900 border-b pb-2">Original Submission Details</h3>
                  
                  <div className="grid grid-cols-1 gap-4">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-600 mb-1">Category & Status</h4>
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-sm"><span className="font-medium">Category:</span> {selectedComplaint.category}</p>
                        <p className="text-sm"><span className="font-medium">Status:</span> {selectedComplaint.status}</p>
                        {selectedComplaint.originalPriority && (
                          <p className="text-sm"><span className="font-medium">Priority:</span> {selectedComplaint.originalPriority}</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="bg-blue-50 p-3 rounded-lg">
                        {selectedComplaint.userInfo && (
                          <>
                            <p className="text-sm"><span className="font-medium">Name:</span> {selectedComplaint.userInfo.name}</p>
                            <p className="text-sm"><span className="font-medium">Email:</span> {selectedComplaint.userInfo.email}</p>
                          </>
                        )}
                        {selectedComplaint.originalContact && (
                          <>
                            <p className="text-sm"><span className="font-medium">Phone:</span> {selectedComplaint.originalContact.phone}</p>
                            {selectedComplaint.originalContact.email && (
                              <p className="text-sm"><span className="font-medium">Alt Email:</span> {selectedComplaint.originalContact.email}</p>
                            )}
                          </>
                        )}
                      </div>
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-gray-600 mb-1">Location Details</h4>
                      <div className="bg-blue-50 p-3 rounded-lg">
                        {selectedComplaint.location && (
                          <>
                            <p className="text-sm"><span className="font-medium">Address:</span> {selectedComplaint.location.address}</p>
                            <p className="text-sm"><span className="font-medium">Municipality:</span> {selectedComplaint.location.municipality}</p>
                          </>
                        )}
                        {selectedComplaint.originalLocation && (
                          <>
                            <p className="text-sm"><span className="font-medium">District:</span> {selectedComplaint.originalLocation.district}</p>
                            <p className="text-sm"><span className="font-medium">Area:</span> {selectedComplaint.originalLocation.area}</p>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-gray-600 mb-1">Timestamps</h4>
                      <div className="bg-blue-50 p-3 rounded-lg">
                        {selectedComplaint.createdAt && (
                          <p className="text-sm">
                            <span className="font-medium">AI Processed:</span> {new Date(selectedComplaint.createdAt).toLocaleString()}
                          </p>
                        )}
                        {selectedComplaint.originalTimestamp && (
                          <p className="text-sm">
                            <span className="font-medium">Originally Submitted:</span> {new Date(selectedComplaint.originalTimestamp).toLocaleString()}
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="mt-6">
                <h3 className="text-lg font-semibold text-gray-800 mb-3">Complete Description</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedComplaint.details && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-600 mb-2">Current Details</h4>
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-gray-800 text-sm">{selectedComplaint.details}</p>
                      </div>
                    </div>
                  )}
                  {selectedComplaint.originalDescription && selectedComplaint.originalDescription !== selectedComplaint.details && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-600 mb-2">Original Description</h4>
                      <div className="bg-gray-50 p-3 rounded-lg">
                        <p className="text-gray-800 text-sm">{selectedComplaint.originalDescription}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Admin Management Panel */}
              {managementMode && <AdminManagementPanel complaint={selectedComplaint} />}
            </div>
          </div>
        </div>
      )}
